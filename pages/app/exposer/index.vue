<template>
  <!-- App container -->
  <v-row no-gutters class="h-100-v secondary">
    <!-- Left Side -->
    <v-col class="full-height">
      ...
      <v-sheet class="d-flex" color="secondary">
        ...
        <v-btn
          ...
          dark
          small
          class="ml-auto mr-3 mt-3 mb-2"
          @click="clickSwitchSidebar"
        >
          <v-icon>
            {{ sidebarOpen ? 'mdi-arrow-expand-right' : 'mdi-arrow-expand-left' }}
          </v-icon>
        </v-btn>
      </v-sheet>
      <!-- End top menu -->
      ...
      <v-row no-gutters class="yellow h-90-p mh-90-p green full-width" dense justify="center" align="stretch">
        <!-- Left Window -->
                  ...
                  <v-row no-gutters justify="center">
                    <v-col cols="">
                      <template v-for="(index) in [0, 1, 2]">
                        <v-col :key="index" cols="auto" class="px-0">
                  ...
                  <v-row no-gutters>
                    <v-col class="">
                      ...
                      <v-col cols="12" class="px-0">
                        ...
                        <v-slider
                          v-model="controls.adaptativeSpread.value"
                          min="0"
                          max="100"
                          thumb-color="grey"
                          thumb-label=""
  ...
  methods: {
    ...
    processModifiers () {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (spreadClip[i][0] !== prevSpreadClip[i][0] || spreadClip[i][1] !== prevSpreadClip[i][1]) {
          changedSpread = true
        }
        if (colorBalance[i] !== prevColorBalance[i]) {
          changedBalance = true
        }
    ...
    clickSwitchSidebar () {
      ...
      const linearAbs = () => {
        const midFactors = Array(256)
        let text = '['
        for (let i = 0; i < 256; i++) {
        // number, a, b -> (number - a) / (b - a)
          if (i <= 127) {
            midFactors[i] = (i - 0) / (127 - 0)
          } else {
            midFactors[i] = (i - 255) / (128 - 255)
          }
          text += midFactors[i]
          if (i < 256 - 1) {
            text += ', '
      ...
      // Google
      const sphericalFactors = () => {
        const midFactors = Array(256)
        for (let i = 0; i < 128; i++) {
          const degree = (i + 1) * (360 / 256)
          const radian = util.degrees_to_radians(degree)
          midFactors[i] = (1 - Math.cos(radian)) * 0.5
          midFactors[255 - i] = midFactors[i]
        }
        util.exportArray(midFactors)
      }
      sphericalFactors()
    },
    viewportZoom (e) {
